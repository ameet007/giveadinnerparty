...
function &load_class($class, $directory = 'libraries', $prefix = 'CI_') {
	...
	foreach ( array (LIBRARY_PATH, APPPATH ) as $path ) {
	...
	if (file_exists ( APPPATH . $directory . '/' . config_item ( 'subclass_prefix' ) . $class . EXT )) {
		echo $name = config_item ( 'subclass_prefix' ) . $class;

		if (class_exists ( $name ) === FALSE) {
			echo APPPATH . $directory . '/' . config_item ( 'subclass_prefix' ) . $class . EXT;
...
/*
	@desc 	convert object to an array
			unset the object properties which are empty
	@arg	$obj	object which is need to converted
*/
function unset_required_fields($obj) {

	//declare varibles
	$arr_fields = array ();

	//convert array to object as we can't access protected properties using any loop
	$arr_casted_obj = ( array ) $obj;

	foreach ( $arr_casted_obj as $key => $val ) {
		//remove the unnecessary data from key. It is there for keys are protected
		preg_match_all ( '/[0-9a-zA-Z_]+/', $key, $matches );
		$key = $matches [0] [0];
		$val = trim ( $val );

		//if value is not empty then fill that key val pair in array
